...
int main(int argc,char *argv[])
{
	...
	//  =======================================================	
	    ...
	    float delta;
		check_stability(sizex,sizey,nz,dt,Lx,Ly,Lz,dx,dy,dz,rx,ry,rz,vp,delta);

        if(delta-1.0>1.0e-6){printf("myid=%d delta=%f unstable!!!\n",myid,delta);exit(0);}
	...
	//  =======================================================	
        ...
        for(n=0;n<shotnum;n++)
        {
               ...
               forward3d_c(sizex,sizey,nz,tmax,dx,dy,dz,dt,rx,ry,rz,vp,dens,xl,xr,yl,yr,zu,zd,
                           ...
                           fm,n,xef,yef,zef,Lx,Ly,Lz,idx,idy,myid,numx,numy,p,vx,vy,vz,
                           fi_px,fi_py,fi_pz,fi_vx,fi_vy,fi_vz,
        ...
        printf("It tooks %f secodes\n", endtime-starttime);}
           ...
           free(rx);free(ry);free(rz);free(vp);free(dens);
...
{
	int ix,iz,iy,t,ii,jj,kk,is,iss;
	...
	sprintf(tmp1,"%s-rec_acou2m2-x%s-y%s.dat",ascn,ascx,ascy);
	strcat(recname,tmp1);
	...
	// (1) up-down-y
		#pragma omp parallel for private(ix,iz,iy,sumx,sumy,sumz,ii,is,iss) 
		...
		// (2) left-right-x
		#pragma omp parallel for private(ix,iy,iz,sumx,sumy,sumz,ii,is,iss) 
		...
		#pragma omp parallel for private(ix,iy,iz,sumx,ii,is)
		...
		for(iz=Lz;iz<nz-Lz;iz++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
				for(ix=Lx;ix<2*Lx;ix++)
				{
					...
					for(ii=0;ii<Lx;ii++)
					{
						sumx=sumx+rx[ii]*(p[iz*sizey*sizex+iy*sizex+(ix+ii+1)]-p[iz*sizey*sizex+iy*sizex+(ix-ii)]);
					}
					sumx=sumx/dx;
					is=iz*sizey*sizex+iy*sizex+ix;
	     	...
	     	#pragma omp parallel for private(iz,iy,ix,sumy,ii,is)
		...
		for(ix=Lx;ix<sizex-Lx;ix++)
		{
			for(iz=Lz;iz<nz-Lz;iz++)
			{
				for(iy=Ly;iy<2*Ly;iy++)
				{
					...
					for(ii=0;ii<Ly;ii++)
					{
						sumy=sumy+ry[ii]*(p[iz*sizey*sizex+(iy+ii+1)*sizex+ix]-p[iz*sizey*sizex+(iy-ii)*sizex+ix]);
					}
					sumy=sumy/dy;
					is=iz*sizey*sizex+iy*sizex+ix;
		...
		#pragma omp parallel for private(ix,iz,iy,sumx,ii,is) 
		for(iz=Lz;iz<nz-Lz;iz++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
		...
		#pragma omp parallel for private(iz,iy,ix,sumz,ii,is,iss) 
		for(iz=Lz-1;iz<nz-Lz;iz++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
				for(ix=Lx;ix<sizex-Lx;ix++)
				{
					...
					for(ii=0;ii<Lz;ii++)
					{
						sumz=sumz+rz[ii]*(p[(iz+ii+1)*sizey*sizex+iy*sizex+ix]-p[(iz-ii)*sizey*sizex+iy*sizex+ix]);
					}
					sumz=sumz/dz;
					is=iz*sizey*sizex+iy*sizex+ix;
...
{
    char strtmp[256];
    FILE *fp=fopen("paramerers3d.txt","r");
    if(fp==0)
    {
        printf("Cannot open the parameters3d.txt file!\n");
        exit(0);
    }
...
//  =========================================================
void split_domains(int nx,int ny,int nxo,int nyo,int Lx,int Ly,int nz,int Lz,int myid,int *avex,int *avey,
		                    ...
		                    int zu,int zd,int nz,int sizex,int sizey)
{
	...
	for(i=xl;i<=xr;i++)
	{
		for(j=yl;j<=yr;j++)
		{
			for(k=zu;k<=zd;k++)
			{
				vp[k*sizey*sizex+j*sizex+i]=modelv[(k-zu)*yef*xef+(j-yl)*xef+i-xl];//(i-xl)*yef*nzo+(j-yl)*nzo+k-zu
...
// ==========================================================
//  This subroutine is used for checking the stability
//  =========================================================	
void check_stability(int sizex,int sizey,int nz,float dt,int Lx,int Ly,int Lz,int dx,int dy,int dz,float *rx,float *ry,float *rz,float *vp,float delta)
 ...
 {
            float sumrx=0.0,sumry=0.0,sumrz=0.0,vmax=0.0;
			int i,j,k;
            for(i=0;i<Lx;i++)
            {
               sumrx=sumrx+fabs(rx[i]);
            }
            for(i=0;i<Ly;i++)
            {
               sumry=sumry+fabs(ry[i]);
            }
            for(i=0;i<Lz;i++)
            {
              sumrz=sumrz+fabs(rz[i]);
            }
            for(k=0;k<nz;k++)
            {
     	        for(j=0;j<sizey;j++)
	            {
					 for(i=0;i<sizex;i++)
					 {
						if(vp[k*sizey*sizex+j*sizex+i]>vmax)
							{
							vmax=vp[k*sizey*sizex+j*sizex+i];
						    }
					 }
				}
            }
         delta=vmax*dt*sqrt(pow(sumrx/dx,2)+pow(sumrz/dz,2)+pow(sumry/dy,2));
