...
/* 
3D acoustic modeling using MPI spliting x direction (The slowest dimension)
(2M,4) SGFD scheme
...
int main(int argc,char *argv[])
{
	...
	if(myid==0)
	{
	   input_parameters(&rectime,&dt,&fm,&shotnum,&dx,&dy,&dz,&nxo,&nyo,&nzo,&Lx,&Ly,&Lz,&numx,&numy,filepathv,filepathr);
	...
	//  =======================================================	
        float *modelv,*modeld,*vp,*dens;
            ...
            input_model_parameters(vp,dens,modelv,modeld,xef,yef,nzo,xl,xr,yl,yr,
		                   zu,zd,nz,sizex,sizey);
	...
	//  FD coefficients 
	//  =======================================================	
     float *Gp;   // (2m,4) FD coefficients for x- ,y- and z directions
...
void forward3d_c(int sizex,int sizey,int nz,int tmax,float dx,float dy,float dz,float dt,
	             ...
	             int Lx,int Ly,int Lz,int idx,int idy,int myid,int numx,int numy,float *p,float *vx,float *vy,
                 float *vz,float *fi_px,float *fi_py,float *fi_pz,float *fi_vx,float *fi_vy,float *fi_vz,int numprocs,
	             int offsetrowxn,int offsetrowyn,char filepathr[40])
{
	int ix,iy,iz,t,ii,is,iss,flag1;
    ...
    MPI_Status statusnap,statusqx[4],statusqy[4]; 
	...
	// define send and receive areas
	int sendlf=Lx,sendrg=sizex-2*Lx;
	int sendup=Ly,senddw=sizey-2*Ly;
	int reclf=0,recrg=sizex-Lx;
	int recup=0,recdw=sizey-Ly;
	...
	for(ix=0;ix<sizex;ix++)
	{
		for(iy=0;iy<sizey;iy++)
		{
			for(iz=0;iz<nz;iz++)
			{
                is=ix*sizey*nz+iy*nz+iz;
				p[is]=0.0;vx[is]=0.0;vy[is]=0.0;vz[is]=0.0;
                fi_vx[is]=0.0;fi_vy[is]=0.0;
				fi_px[is]=0.0;fi_py[is]=0.0;fi_vz[is]=0.0;fi_pz[is]=0.0;
	...
	for(ix=0;ix<insizex;ix++)
	{
	    for(iy=0;iy<Ly;iy++)
	    {
	        for(iz=0;iz<nz;iz++)
	        {
	            rareaup[ix*Ly*nz+iy*nz+iz]=0.0;
	            rareadw[ix*Ly*nz+iy*nz+iz]=0.0;            
	        }
	    }
	}
	#pragma omp parallel for private(ix,iy,iz) 
	for(ix=0;ix<Lx;ix++)
	{
	    for(iy=0;iy<insizey;iy++)
	    {
	        for(iz=0;iz<nz;iz++)
	        {
	            rarealf[ix*insizey*nz+iy*nz+iz]=0.0;
	            rarearg[ix*insizey*nz+iy*nz+iz]=0.0;            
	        }
	    }
	}
	...
	sprintf(tmp1,"%s-record2m4-x%s.dat",ascn,ascx);
	strcat(recname,tmp1);
    ...
    int shotpos=(shotxlc[n])*sizey*nz+shotylc[n]*nz+shotzlc[n];	
	...
	for(t=0;t<tmax;t++)     ///// time iteration starts
	{
		if(myid==0&&t%10==0)
		{
		  ...
		  for(ix=0;ix<xef;ix++)
		  {
			for(iy=0;iy<yef;iy++)
			{
			     rec[ix*yef+iy]=p[(ix+xl)*sizey*nz+(iy+yl)*nz+recg+zu];
			}
		  }
		...
		// (1) up-down-y
		#pragma omp parallel for private(ix,iy,iz,sumx,sumy,sumz,ii,is,iss,index) 
			     ...
			     for(iy=Ly;iy<2*Ly;iy++)   
			        {
					 ...
					 for(ii=0;ii<Lx;ii++)
					 {
						 sumx=sumx+Gp[index*Lpx+ii]*(vx[(ix+ii)*sizey*nz+iy*nz+iz]-vx[(ix-ii-1)*sizey*nz+iy*nz+iz]);
					 }					  
					     sumx=sumx+Gp[index*Lpx+Lx]*(vx[ix*sizey*nz+(iy+1)*nz+iz]-vx[(ix-1)*sizey*nz+(iy+1)*nz+iz]+vx[ix*sizey*nz+(iy-1)*nz+iz]-vx[(ix-1)*sizey*nz+(iy-1)*nz+iz]);
					     sumx=sumx+Gp[index*Lpx+Lx+1]*(vx[ix*sizey*nz+iy*nz+iz+1]-vx[(ix-1)*sizey*nz+iy*nz+iz+1]+vx[ix*sizey*nz+iy*nz+iz-1]-vx[(ix-1)*sizey*nz+iy*nz+iz-1]);
					     sumx=sumx/dx;
					 for(ii=0;ii<Ly;ii++)
					 {
					     sumy=sumy+Gp[index*Lpy+ii]*(vy[ix*sizey*nz+(iy+ii)*nz+iz]-vy[ix*sizey*nz+(iy-ii-1)*nz+iz]);      
					 }
					     sumy=sumy+Gp[index*Lpy+Ly]*(vy[(ix+1)*sizey*nz+iy*nz+iz]-vy[(ix+1)*sizey*nz+(iy-1)*nz+iz]+vy[(ix-1)*sizey*nz+iy*nz+iz]-vy[(ix-1)*sizey*nz+(iy-1)*nz+iz]);
					     sumy=sumy+Gp[index*Lpy+Ly+1]*(vy[ix*sizey*nz+iy*nz+iz+1]-vy[ix*sizey*nz+(iy-1)*nz+iz+1]+vy[ix*sizey*nz+iy*nz+iz-1]-vy[ix*sizey*nz+(iy-1)*nz+iz-1]);
                         sumy=sumy/dy;
					 for(ii=0;ii<Lz;ii++)
					 {
						 sumz=sumz+Gp[index*Lpz+ii]*(vz[ix*sizey*nz+iy*nz+iz+ii]-vz[ix*sizey*nz+iy*nz+iz-ii-1]);
					 }
					     sumz=sumz/dz;
		...
		// update the p values of sending areas
		#pragma omp parallel for private(ix,iy,iz,iss)
		for(ix=0;ix<insizex;ix++)
		{
			for(iy=0;iy<Ly;iy++)
			{
				for(iz=0;iz<nz;iz++)
				{
					iss=ix*Ly*nz+iy*nz+iz;
					sareaup[iss]=p[(ix+Lx)*sizey*nz+(iy+Ly)*nz+iz];
					sareadw[iss]=p[(ix+Lx)*sizey*nz+(iy+sizey-2*Ly)*nz+iz];
				}
			}
		}
		#pragma omp parallel for private(ix,iy,iz,iss)
		for(ix=0;ix<Lx;ix++)   
		{
			for(iy=0;iy<insizey;iy++)
			{
				for(iz=0;iz<nz;iz++)
				{
					iss=ix*insizey*nz+iy*nz+iz;
					sarealf[iss]=p[(ix+Lx)*sizey*nz+(iy+Ly)*nz+iz];
					sarearg[iss]=p[(ix+sizex-2*Lx)*sizey*nz+(iy+Ly)*nz+iz]; 
				}
			}
		}
		...
		for(ix=0;ix<insizex;ix++)
		{
			for(iy=0;iy<Ly;iy++)
			{
				...
				{
					iss=ix*Ly*nz+iy*nz+iz;
					p[(ix+Lx)*sizey*nz+iy*nz+iz]=rareaup[iss];
					p[(ix+Lx)*sizey*nz+(iy+sizey-Ly)*nz+iz]=rareadw[iss];
				}
			}
		}
		//(2) left-right areas
		...
		for(ix=0;ix<Lx;ix++)   
		{
			for(iy=0;iy<insizey;iy++)
			{
				...
				{
					iss=ix*insizey*nz+iy*nz+iz;
					p[ix*sizey*nz+(iy+Ly)*nz+iz]=rarealf[iss];
					p[(ix+sizex-Lx)*sizey*nz+(iy+Ly)*nz+iz]=rarearg[iss]; 
				}
			}
		}
		...
		for(iz=Lz;iz<nz-Lz;iz++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
				for(ix=Lx;ix<2*Lx;ix++)
				{
					...
					for(ii=0;ii<Lx;ii++)
					{
					 sumx=sumx+Gp[index*Lpx+ii]*(p[(ix+ii+1)*sizey*nz+iy*nz+iz]-p[(ix-ii)*sizey*nz+iy*nz+iz]);
					}
					 sumx=sumx+Gp[index*Lpx+Lx]*(p[(ix+1)*sizey*nz+(iy+1)*nz+iz]-p[ix*sizey*nz+(iy+1)*nz+iz]+p[(ix+1)*sizey*nz+(iy-1)*nz+iz]-p[ix*sizey*nz+(iy-1)*nz+iz]);
					 sumx=sumx+Gp[index*Lpx+Lx+1]*(p[(ix+1)*sizey*nz+iy*nz+iz+1]-p[ix*sizey*nz+iy*nz+iz+1]+p[(ix+1)*sizey*nz+iy*nz+iz-1]-p[ix*sizey*nz+iy*nz+iz-1]);			
					 sumx=sumx/dx;
		...
		for(ix=Lx;ix<sizex-Lx;ix++)
		{
			for(iz=Lz;iz<nz-Lz;iz++)
			{
				for(iy=Ly;iy<2*Ly;iy++)
				{
					...
					for(ii=0;ii<Ly;ii++)
					{
					 sumy=sumy+Gp[index*Lpy+ii]*(p[ix*sizey*nz+(iy+ii+1)*nz+iz]-p[ix*sizey*nz+(iy-ii)*nz+iz]);
					}
					sumy=sumy+Gp[index*Lpy+Ly]*(p[(ix+1)*sizey*nz+(iy+1)*nz+iz]-p[(ix+1)*sizey*nz+iy*nz+iz]+p[(ix-1)*sizey*nz+(iy+1)*nz+iz]-p[(ix-1)*sizey*nz+iy*nz+iz]);
					sumy=sumy+Gp[index*Lpy+Ly+1]*(p[ix*sizey*nz+(iy+1)*nz+iz+1]-p[ix*sizey*nz+iy*nz+iz+1]+p[ix*sizey*nz+(iy+1)*nz+iz-1]-p[ix*sizey*nz+iy*nz+iz-1]);
					sumy=sumy/dy;
		...
		for(ix=0;ix<insizex;ix++)
		{
			for(iy=0;iy<Ly;iy++)
			{
				...
				{
					iss=ix*Ly*nz+iy*nz+iz;
					sareaup[iss]=vy[(ix+Lx)*sizey*nz+(iy+Ly)*nz+iz];
					sareadw[iss]=vy[(ix+Lx)*sizey*nz+(iy+sizey-2*Ly)*nz+iz];
				}
			}
		}
		#pragma omp parallel for private(iz,iy,ix,iss)
		for(ix=0;ix<Lx;ix++)   
		{
			for(iy=0;iy<insizey;iy++)
			{
				...
				{
					iss=ix*insizey*nz+iy*nz+iz;
					sarealf[iss]=vx[(ix+Lx)*sizey*nz+(iy+Ly)*nz+iz];
					sarearg[iss]=vx[(ix+sizex-2*Lx)*sizey*nz+(iy+Ly)*nz+iz]; 
				}
			}
		}
		...
		#pragma omp parallel for private(iz,iy,ix,sumx,ii,is,index) 
		for(ix=2*Lx;ix<sizex-2*Lx;ix++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
		...
		#pragma omp parallel for private(iz,iy,ix,sumz,ii,is,iss,index) 
		for(ix=Lx;ix<sizex-Lx;ix++)
		{
			for(iy=Ly;iy<sizey-Ly;iy++)
			{
				for(iz=Lz-1;iz<nz-Lz;iz++)
				{
					...
					for(ii=0;ii<Lz;ii++)
					{
						sumz=sumz+Gp[index*Lpz+ii]*(p[ix*sizey*nz+iy*nz+iz+ii+1]-p[ix*sizey*nz+iy*nz+iz-ii]);///////////
					}
					sumz=sumz+Gp[index*Lpz+Lz]*(p[ix*sizey*nz+(iy+1)*nz+iz+1]-p[ix*sizey*nz+(iy+1)*nz+iz]+p[ix*sizey*nz+(iy-1)*nz+iz+1]-p[ix*sizey*nz+(iy-1)*nz+iz]);
					sumz=sumz+Gp[index*Lpz+Lz+1]*(p[(ix+1)*sizey*nz+iy*nz+iz+1]-p[(ix+1)*sizey*nz+iy*nz+iz]+p[(ix-1)*sizey*nz+iy*nz+iz+1]-p[(ix-1)*sizey*nz+iy*nz+iz]);				
					sumz=sumz/dz;
		...
		for(ix=0;ix<insizex;ix++)
		{
			for(iy=0;iy<Ly;iy++)
			{
				...
				{
					iss=ix*Ly*nz+iy*nz+iz;
					vy[(ix+Lx)*sizey*nz+iy*nz+iz]=rareaup[iss];
					vy[(ix+Lx)*sizey*nz+(iy+sizey-Ly)*nz+iz]=rareadw[iss];
				}
			}
		}
		#pragma omp parallel for private(iz,iy,ix,iss) 
		for(ix=0;ix<Lx;ix++)   
		{
			for(iy=0;iy<insizey;iy++)
			{
				...
				{
					iss=ix*insizey*nz+iy*nz+iz;
					vx[ix*sizey*nz+(iy+Ly)*nz+iz]=rarealf[iss];
					vx[(ix+sizex-Lx)*sizey*nz+(iy+Ly)*nz+iz]=rarearg[iss]; 
				}
			}
		}
...
//  =========================================================
void input_parameters(float *rectime,float *dt,float *fm,int *shotnum,float *dx,float *dy,float *dz,int *nxo,int *nyo,int *nzo,int *Lx,
		              int *Ly,int *Lz,int *numx,int *numy,char filepathv[40],char filepathr[40])
{
    char strtmp[256];
    FILE *fp=fopen("parameters3d.txt","r");
    if(fp==0)
    {
        printf("Cannot open the parameters.txt file!\n");
        exit(0);
    }
...
//  =========================================================
void input_model_parameters(float *vp,float *dens,float *modelv,float *modeld,int xef,int yef,int nzo,
			               int xl,int xr,int yl,int yr,int zu,int zd,int nz,int sizex,int sizey)
{
    int i,j,k;
	for(i=xl;i<=xr;i++)
	{
		for(j=yl;j<=yr;j++)
		{
			for(k=zu;k<=zd;k++)
			{
				vp[i*sizey*nz+j*nz+k]=modelv[(i-xl)*yef*nzo+(j-yl)*nzo+k-zu];
				dens[i*sizey*nz+j*nz+k]=modeld[(i-xl)*yef*nzo+(j-yl)*nzo+k-zu];
			}
		}
	}
	...
	for(i=xl;i<=xr;i++)
	{
	        for(j=yl;j<=yr;j++)
		{
			for(k=0;k<zu;k++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[i*sizey*nz+j*nz+zu];
				dens[i*sizey*nz+j*nz+k]=dens[i*sizey*nz+j*nz+zu];
			}
			for(k=zd+1;k<nz;k++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[i*sizey*nz+j*nz+zd];
				dens[i*sizey*nz+j*nz+k]=dens[i*sizey*nz+j*nz+zd];
			}
		}
		for(k=0;k<nz;k++)
		{
			for(j=0;j<yl;j++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[i*sizey*nz+yl*nz+k];
				dens[i*sizey*nz+j*nz+k]=dens[i*sizey*nz+yl*nz+k];
			}

			for(j=yr+1;j<sizey;j++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[i*sizey*nz+yr*nz+k];
				dens[i*sizey*nz+j*nz+k]=dens[i*sizey*nz+yr*nz+k];
			}
		}
	}
	for(i=0;i<xl;i++)
	{
		for(j=0;j<sizey;j++)
		{
			for(k=0;k<nz;k++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[xl*sizey*nz+j*nz+k];
				dens[i*sizey*nz+j*nz+k]=dens[xl*sizey*nz+j*nz+k];
			}
		}
	}
	...
	for(i=xr+1;i<sizex;i++)
	{
		for(j=0;j<sizey;j++)
		{
			for(k=0;k<nz;k++)
			{
				vp[i*sizey*nz+j*nz+k]=vp[xr*sizey*nz+j*nz+k];
				dens[i*sizey*nz+j*nz+k]=dens[xr*sizey*nz+j*nz+k];
			}
		}
	}
...
// ==========================================================
//  This subroutine is used for calculating CPML parameters
//  =========================================================
                  ...
                  float *modelv,float *modeld)
{
     ...
     sprintf(tmp1,"x%s-vp.dat",ascx);
     sprintf(tmp2,"x%s-rho.dat",ascx);
